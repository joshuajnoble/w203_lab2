...
__Overview.__\
Prior to the 2016 presidential election, members of the US Congress publically disclosed the existence of attempted Russian interference activities. Robert Mueller, a former FBI director, led a Special Counsel Investigation from May 2017 to March 2019. This survey occurred while this Mueller's investigation was ongoing.
...
Both questions have potential to provide some insight into the question at hand, assuming that respondents provided honest answers.  The survey question is close to the question at hand.  Assessing the available data will help frame which ones best inform the analysis.

__Explorataory Data Analysis.__\

```{r}

russia_involvement <- data.frame(anes_data$pid1d, anes_data$turnout18, anes_data$russia16, anes_data$coord16)
colnames(russia_involvement) <- c("party", "voted", "russia_1", "russia_2")
russia_involvement<- russia_involvement[which(russia_involvement$party == c(3)),] # isolate independent voters
...
```{r}
t.test(russia_involvement$russia_1, alternative = c("two.sided"), mu = 1)
```

```{r}
t.test(russia_involvement$russia_2, alternative = c("two.sided"), mu = 1)
```
...
__Statistical Significance.__  The p-values for these tests, p = 8.74e-13 and 8.708e-11 both support rejecting the null hypothesis.
__Practical Significance.__  The Cohen's d effect size value, d = -3.633, indicates that there is a large effect present between the two survey questions.
...
```{r}
library(knitr)
Independent_Russia <- c(1.423, 1.487)
Democrats_Russia <- c(1.025, 1.058)
Republicans_Russia <- c(1.848, 1.962)
...
Survey respondents consistently aligned to their party affiliation - Democrats aligned to the presence of influence, while Republicans were skewed towards the opinion of no involvment.  Independent affiliated respondents roughly split the diffence between the two extremes.
\pagebreak
...
__Operationalization.__\
No set of questions directly answers this question. this question was approached in two parts:  identify voters in 2016 and 2018, and then identify what makes them fearful or angry.  To identify voters, the questions addressing voter turnout in 2016 and 2018 provide the best initial insight on voting respondents.  
...
There are no non-responses, only respondents who are not sure if they voted, as per the codebook. We can filter these to find respondents who are sure they voted in 2016 and in 2018.  We can see an increase of 1 vote from 2018 to 2016, so it appears that the principle assumption of this question, that there was an increase in votes 2018 vs 2016, is incorrect. 
