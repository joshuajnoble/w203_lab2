...
```
A particularly interesting feature of the data is that the responses almost completely align to party lines.
...
![Russia by party](q3_by_party.png)

Data for respondents that voted for each of the identified parties - Republican, Democrat, and Independent, is depicted in the table below.
...
```{r}
voter_turnout_16_18 = anes_data[which (anes_data$turnout16==1 & anes_data$turnout18 < 4),] # isolate 2016 and 2018 voters
summary(voter_turnout_16_18)
```
...
```{r}
angry = voter_turnout_16_18$geangry  #create vector for angry responses
afraid = voter_turnout_16_18$geafraid # create vector for afraid responses
voter_emotions = data.frame(angry,afraid)
```

```{r}
library(BSDA)
SIGN.test(voter_emotions$angry, voter_emotions$afraid, md = 0, alternative = "two.sided", conf.level = 0.95)
```

```{r}
cor(voter_emotions$angry, voter_emotions$afraid, method = "spearman")
```

__Outcome.__

__Statistical significance.__ The sign test p-value of 2.22e-16 indicates that we can reject our null hypothesis that the median value of fear and anger for 2018 election voters is zero.  The correlation does indicate that there is a fairly strong positive correlation between anger and fear for the voters surveyed.
...
```{r}
wilcox.test(voter_emotions$angry, voter_emotions$afraid, alternative = "two.sided", mu = 0, paired = TRUE, conf.level = 0.95)
```

__Practical significance.__ 

```{r}
mean_angry = mean(voter_emotions$angry)
sd_angry = sd(voter_emotions$angry)
zscale_angry = scale(voter_emotions$angry)[,1]
summary(zscale_angry)
angry_effect = 0.6614/sqrt(length(voter_emotions$angry))
print(angry_effect)
mean_afraid = mean(voter_emotions$afraid)
sd_afraid = sd(voter_emotions$afraid)
zscale_afraid = scale(voter_emotions$afraid) [,1]
summary(zscale_afraid)
afraid_effect = 0.9662/sqrt(length(voter_emotions$afraid))
print(afraid_effect)
```
...
```{r}
library(knitr)
angryeffect = c(angry_effect, mean_angry)
afraideffect = c(afraid_effect, mean_afraid)
